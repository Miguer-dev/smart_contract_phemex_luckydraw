...
contract LuckyDraw is VRFV2WrapperConsumerBase, Ownable {
    ...
    event RequestDraw(uint256 round, uint256 requestId, uint256 paid, uint256 maxSoulpassId);
    event FulfillDraw(uint256 round, uint256 soulpassId, address soulpassOwner, uint256 payment);
    ...
    mapping(uint256 => FulfillDrawStatus) public results;
    mapping(uint256 => uint256) _winnerRecord;
    //
     ...
     * 
     * _soulpassAddress = 0xCA20888A6F99EF8691e04D7e75f7b5faE8fa7591
     * _totalRounds = 10
     * _luckyAmount = 10000000000000000000 = 10 ether
     * _startTimestamp = 1693440000 
     * _endTimestamp = 1693562400 
     * _maxSoulpassId = 5145
     */
    constructor(
    ...
    function requestDraw(uint256 _round, uint32 _callbackGasLimit, uint256 _maxSoulpassId)
        external
        onlyOperator
    ...
    function getRequestStatus(uint256 _requestId)
        external
        view
        returns (uint256 paid, bool fulfilled, uint256[] memory randomWords, uint256 soulpassId)
    {
    ...
    function getFulfillStatus(uint256 _round)
        external
        view
